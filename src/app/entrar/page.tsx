...
function EntrarContent() {
  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    try {
      ...
      if (otpError) {
        setMagicLinkError("Erro ao enviar o link. Tenta novamente.");
      } else {
        setMagicLinkSent(true);
      }
    } catch {
      setMagicLinkError("Erro de ligação. Tenta novamente.");
    }
  ...
  return (
    <section className="bg-cream px-6 py-24 sm:py-32">
      <div className="mx-auto max-w-md">
        ...
        {/* Magic link - secondary method */}
        <div className="mt-6">
          {magicLinkSent ? (
            <div className="rounded-lg border border-sage/30 bg-sage/5 p-6 text-center">
            ...
            <>
              ...
              >
                Entrar com link mágico (via email)
                <svg
                  className={`h-4 w-4 transition-transform ${showMagicLink ? "rotate-180" : ""}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth={1.5}
                  stroke="currentColor"
              ...
              {showMagicLink && (
                ...
                >
                  ...
                  <button
                    type="submit"
                    disabled={magicLinkLoading}
                    className={`w-full rounded-lg border border-sage bg-white px-8 py-3 font-sans text-[0.8rem] font-medium uppercase tracking-[0.12em] text-sage transition-colors hover:bg-sage/5 ${magicLinkLoading ? "opacity-60" : ""}`}
                  >
