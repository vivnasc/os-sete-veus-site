...
-- Executar no Supabase SQL Editor (uma só vez)
-- ============================================================
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'diario_espelho' AND policyname = 'Users can read own diario_espelho') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'diario_espelho' AND policyname = 'Users can insert own diario_espelho') THEN
...
CREATE INDEX IF NOT EXISTS idx_diario_espelho_user_date
  ON diario_espelho(user_id, created_at DESC);


-- 2. ECOS GUARDADOS — Colecção pessoal de ecos que tocaram
-- Nota: eco_id é UUID sem FK constraint para funcionar
-- independentemente de a tabela ecos já existir ou não
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ecos_guardados' AND policyname = 'Users can read own ecos_guardados') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ecos_guardados' AND policyname = 'Users can insert own ecos_guardados') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'ecos_guardados' AND policyname = 'Users can delete own ecos_guardados') THEN
...
CREATE INDEX IF NOT EXISTS idx_ecos_guardados_user
